    ...
    <view>
      ...
      <view class="comment hot" v-if="hot.length">
        <view class="comment_title">
          <view class="inco">火</view>
          <text class="commment_text">最热评论</text>
        </view>
          ...
          <view class="comment_item" v-for="item in hot" :key="item.id">
            ...
            <view class="comment_user">
              ...
              <view class="user_badge">
                <image v-for="item2 in item.user.title" :key="item2.icom" :src="item2.icon"></image>
              </view>
            </view>
            <!-- 评论数据 -->
            <view class="comment_desc">
              ...
              <view class="comment_like">
                <text class="iconfont icon-dianzan">{{tiem.size}}</text>
              </view>
            </view>
          </view>
        </view>
      ...
      <view class="comment new" v-if="comment.length">
        <view class="comment_title">
          <view class="inco">火</view>
          <text class="commment_text">最新评论</text>
        </view>
        <view class="comment_list">
      ...
      <!-- 下载开始 -->
      <view class="download">
        <view class="download_btn" @click="hanleDownload">下载图片</view>
      </view>
      <!-- 下载结束 -->
    </view>
</template>
...
export default {
    ...
    methods: {
      ...
      handleSwiperAction (e) {
         ...
         * */ 
        const { imgList } = getApp().globalData
        if (e.directory === 'left' && this.imgIndex < imgList.length - 1) {
          this.imgIndex++
          this.getData()
        } else if(e.directory === 'right' && this.imgIndex > 0) {
          this.imgIndex--
          this.getData()
        } else {
          uni.showToast({
